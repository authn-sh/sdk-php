...
- `JwtTemplatesManager` — BAPI binding for `/v1/jwt-templates` (`list`, `create`, `get`, `update`, `delete`) plus the `JwtTemplate` DTO (`id`, `name`, `claims`, `lifetime`, `allowedClockSkew`, `signingAlgorithm`, `createdAt`, `updatedAt`). Accessible via `$client->jwtTemplates()`. `name` + `signing_algorithm` are immutable after create; `delete()` raises `409 jwt_template_in_use` while active sessions still reference the template. Write-only `custom_signing_key` is accepted on `create`/`update` and never returned by GET.
- `OauthApplicationsManager` — BAPI binding for `/v1/oauth-applications` (`list`, `create`, `get`, `update`, `delete`, `rotateSecret`) plus the `OauthApplication` DTO (`id`, `name`, `clientId`, `callbackUrls`, `scopes`, `isPublic`, `createdAt`, `updatedAt`) and `OauthApplicationWithSecret` (carries the plaintext `clientSecret` returned exactly once on create of a confidential client or on `rotateSecret`). Accessible via `$client->oauthApplications()`. `rotateSecret()` rejects public clients with `409 oauth_application_public_client`; `delete()` cascade-revokes every outstanding `AuthorizationGrant` rooted at the application.
...
- `EnterpriseAccount` DTO — read-only shape for the per-user link to an `EnterpriseConnection`. Carries `provider_user_id`, `email_address`, `verified`, `public_metadata` (raw IdP attributes that did not map to a `User` field), `linked_at`, and `last_signed_in_at`. Exposed by the embedded `enterprise_accounts[]` array on `User` and by the `EnterpriseAccountsManager`.
- `EnterpriseAccountsManager` — BAPI binding for `/v1/enterprise-accounts` (`list`, `get`, `delete`) plus `EnterpriseAccountsListParams` with `user_id` and `enterprise_connection_id` filters. Accessible via `$client->enterpriseAccounts()`. No `create` / `update` — enterprise accounts are provisioned through the SSO callback, not via the BAPI. `delete()` is the admin unlink — the next sign-in via the same `EnterpriseConnection` will JIT-provision a fresh row if policy allows.
